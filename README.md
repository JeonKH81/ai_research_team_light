...
> ### 먼저 이것부터 — 없으면 아무것도 돌지 않습니다
> 이 폴더는 **Claude Code 위에서** 돕니다. 폴더만 받아서는 아무 일도 일어나지 않습니다.
> 실습 전에 **[준비 문서](docs/prepare.md)** 의 셋을 끝내 오세요 (15분):
> **① Claude 를 쓸 창** (Claude 앱의 Code 탭 · VS Code 확장 · 터미널 중 하나, **유료 계정 필요**)
> **② Python 3.9 이상** (윈도우는 설치 때 `Add python.exe to PATH` 켜기)
> **③ 이 폴더 내려받기**
...
flowchart TB
    ...
    FILES["파일에 남는 상태<br/>registry.yaml · PROJECT.md · 연구 질문 후보 · 현황판"]
...
3. **현황판은 손으로 그리지 않는다.** 등록부와 프로젝트 문서에서 매번 다시 만들어집니다 —
   화면을 고치는 것이 아니라 프로젝트 문서를 고칩니다.
...
**먼저 [준비 문서](docs/prepare.md)를 끝냈는지 확인하세요.** 그다음 연구실 폴더를 Claude Code 로 열고(앱의 Code 탭·VS Code·터미널 중 무엇이든) [실습 프롬프트](docs/prompts.md)를 순서대로 입력하면 연구실 설정 → 내 연구 등록 → **연구원 하나를 내가 만든다** → 팀장 창 → 연구원·Critic → 보고·현황판 → 발굴까지 한 바퀴 돕니다. 명령어를 직접 칠 일은 없습니다. 종이 [워크시트](docs/worksheet.md)를 먼저 채우면 빠릅니다.
