...
VS Code 로 폴더를 열고 Claude Code 창에 [실습 프롬프트](docs/prompts.md)를 순서대로 입력하면 연구실 설정 → 내 연구 등록 → **연구원 하나를 내가 만든다** → 팀장 창 → 연구원·Critic → 보고·현황판 → 발굴까지 한 바퀴 돕니다. 명령어를 직접 칠 일은 없습니다. 종이 [워크시트](docs/worksheet.md)를 먼저 채우면 빠릅니다.
