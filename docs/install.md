...
**터미널 없이 하려면 (권장, 핸즈온):** GitHub 페이지의 초록 **Code → Download ZIP** 으로 받아 압축을 풀고, **VS Code → 파일 → 폴더 열기**로 그 폴더를 연 뒤 Claude Code 창을 엽니다(Claude 앱의 Code 탭도 같습니다). 폴더 이름이 `ai_research_team_light-main` 이어도 됩니다. 이후 단계는 앱 안의 대화창에서 그대로 진행합니다 — 명령은 앞에 `!` 를 붙여 치거나, 총괄팀장에게 "설치 확인 돌려 줘"라고 하면 됩니다.
