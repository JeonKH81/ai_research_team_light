...
| 무엇 | 확인하는 법 | 없으면 |
|---|---|---|
| macOS 또는 Linux | — | 둘 다 됩니다 |
| Python 3.9 이상 | `python3 --version` | [python.org](https://www.python.org/downloads/) 에서 설치합니다 |
| Claude Code | `claude --version` | 설치한 뒤 Claude 계정으로 로그인합니다. **별도의 열쇠는 필요 없습니다** |
| 인터넷 | — | 없어도 대부분 돕니다. 멈추는 것은 문헌 수집뿐입니다 |
...
설치 직후에는 **설정 문답 한 줄만 `✗`** 인 것이 정상입니다 — 4단계에서 채웁니다.
...
| `source .venv/bin/activate` (전용 공간을 만든 경우) | `.venv\Scripts\activate` |

Claude Code 는 윈도우에서도 같은 명령(`claude`)입니다. 이 문서의 나머지 명령은 앞의 `python3` 를 `python` 으로 바꿔 읽으면 됩니다.
...
```bash
python3 install.py  (인증서 부품을 넣는다 — 맥에서 그래도 안 되면 zsh _team/scripts/fix_certificates.sh)
```
